...
access_tokenjson = resp.json()
access_token = access_tokenjson['token']
...
try:
    ...
    print(imgdir + ' ditrectory exist')
print('Creating image structure in: ' + imgdir)

configjson = resp.json()
config = configjson['config']['digest']
...
for layer in layers:
    ...
    try:
        os.mkdir(layerdir)
        print('layer directory created: '+layerdir)
    except:
        print(layerdir + ' directory exist')
    ...
    if os.path.exists(layerdir + '/layer.tar'): # this layer is alredy downloaded and decompressed to layer.tar.
        print('this layer is alredy downloaded and decompressed to layer.tar.')
    else:
        # Creating layer.tar file
        sys.stdout.write(ublob[7:19] + ': Downloading...')
        sys.stdout.flush()
        
        file_auth_head=auth_head.copy()
        
        layerfilepath = Path(layerdir + '/layer_gzip.tar')
        layerfilesize=0
        if( os.path.exists(str(layerfilepath)) ):
            layerfilesize = layerfilepath.stat().st_size
            file_auth_head['Range']='bytes=%d-' % layerfilesize
            print('layer_gzip.tar file found in this layer. resume downloading this file from %d  ...'%layerfilesize)
            
        bresp = requests.get('https://{}/v2/{}/blobs/{}'.format(registry, repository, ublob), headers=file_auth_head, stream=True, verify=False)
        
        if(bresp.status_code==416): #request range is bigger than server file size. going to next layer
            print('layer_gzip.tar file was complete.')
        else:
            if (bresp.status_code != 200 and bresp.status_code != 206): # FIXME: if 401 (token expired), this will throw an error on 'urls' keyword
                bresp = requests.get(layer['urls'][0], headers=file_auth_head, stream=True, verify=False) # will fail if token expired
                
                if(bresp.status_code==416):
                    continue
                if (bresp.status_code != 200 and bresp.status_code != 206):
            ...
            with open(layerdir + '/layer_gzip.tar', "ab") as file: # open file as 'ab' (append binary mode)
...
print('creating manifest.json file.')
...
print('craeting image tar file and clean tmp folder.')
