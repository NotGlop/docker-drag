...
def command_parser(command):
    """
    Parse user input to get registry, repository, image, and tag or digest.

    :param command: Input in form of [[registry/]repository/]image[:tag|@digest]
    :return: registry, repository, image, tag, digest
    """
    # Get url, name, image, tag or digest
    imgtag = command.split('/')[-1]
    try:
        image, tag = imgtag.split('@')
    except ValueError:
        try:
            image, tag = imgtag.split(':')
        except ValueError:
    ...
    try:
        registry, repository, _ = command.split('/')
    except ValueError:
        registry = DEFAULT_URL
        try:
            repository, _ = command.split('/')
        except ValueError:
...
if __name__ == '__main__':
    ...
    parser.add_argument('image', metavar='image', nargs=1,
                        help='image to download formatted as: [[registry/]repository/]image[:tag|@digest]')
    args = parser.parse_args()

    # Parse user input
    url, repo, img, tag = command_parser(args.image[0])
